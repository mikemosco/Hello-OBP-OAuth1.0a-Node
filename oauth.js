...
app.get("/connect", function (req, res) {
  consumer.getOAuthRequestToken(function (
    ...
    if (error) {
      res
        .status(500)
        .send("Error getting OAuth request token : " + util.inspect(error));
    } else {
  ...
  consumer.getOAuthAccessToken(
    ...
    function (error, oauthAccessToken, oauthAccessTokenSecret, result) {
      ...
      if (error) {
        ...
        res
          .status(500)
          .send("Error getting OAuth access token : " + util.inspect(error));
      } else {
        //error is now undefined
        req.session.oauthAccessToken = oauthAccessToken;
        req.session.oauthAccessTokenSecret = oauthAccessTokenSecret;
        res.redirect(config.redirectUrlApp);
      }
    }
  );
});
...
function apiGet(req, res) {
  const bankId = "ms.01.mx";
  // /obp/v4.0.0/banks/BANK_ID/accounts
  const url = apiHOst + "";
  const accessToken = req.session.oauthAccessToken;
  const accessTokenSecret = req.session.oauthAccessTokenSecret;
  ...
  consumer.post(
    ...
    function (error, data, response) {
      ...
      try {
        ...
        console.log(err);
        message =
          "Something went wrong creating a transaction request - did you supply the correct values?";
      }
...
app.get("/loadCustomers", function (req, res) {
  ...
  console.log(
    "before customer loop. There are " + customers.length + " customers."
  );
  ...
  customers.forEach(function processCustomer(customer) {
    ...
    consumer.get(
      ...
      function getUserForCustomer(error, data) {
        ...
        console.log(
          "I got customer with email , number : " +
            customer.email +
            " , " +
        ...
        consumer.post(
          ...
          function (error, data) {
            ...
            var parsedData = JSON.parse(data);
            console.log(
              "response from postCustomerUrl: " + JSON.stringify(parsedData)
            );
          }
        ); // End post customer
      }
